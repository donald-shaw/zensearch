...
class RequestHandler(typeInfos: TypeInfos, refs: LookupRefs, childProvider: ChildProvider)
  ...
  def waitForBaseResponse(request: Request, replyTo: ActorRef): Receive = LoggingReceive {
    case resp@Response(req, _, _, data, _) =>
      ...
      if (linked.isEmpty) {
        ...
        linked.foreach { case (req, _) => reqActor(childProvider, context, refs) ! req }
  ...
  def waitForLinkedResponses(baseResp: Response, remaining: List[(Request, CrossLink)], replyTo: ActorRef,
                             returned: List[(Response, CrossLink)] = Nil): Receive = LoggingReceive {
    case resp: Response =>
      log.debug(s"FormattingActor - got linked Response = ${resp}\n\n, replyTo = $replyTo\n")
      remaining.find(_._1.id == resp.request.id) match {
        case Some(found) =>
          val left = remaining.filterNot(_ == found)
          val updated = (resp, found._2) :: returned
          if (left.isEmpty) {
            replyTo ! baseResp.addCrossLinks(updated)
            context.stop(self)
          } else {
            context.become(waitForLinkedResponses(baseResp, left, replyTo, updated))
          }
  ...
  private def findLinked(data: List[DataEntry]) = {
    for {
      ...
      value <- xlinkRef match {
        ...
        case _ => None // @TODO - DSS: not handling arrays at present - drop any other json type here
      }
    } yield (Request(crit, Some(value)), xlinkData)
  }
}
