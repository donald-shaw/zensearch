...
class ZenSearchIntegrationTestBase extends FunSuite with TestKitBase with ImplicitSender with BeforeAndAfterAll {
  ...
  test("Integration test - retrieve user with specific name") {
    ...
    expectMsgPF() {
      case FormatResult(entries) =>
        ...
        assert(entries.head.text.text.contains(""""subject": "A Catastrophe in Micronesia""""))
    }
  }
  ...
  test("Integration test - retrieve tickets with specific submitter id") {
    ...
    expectMsgPF() {
      case FormatResult(entries) =>
        ...
        assert(entries.head.text.text.contains("A Drama in Australia"))
  ...
  test("Integration test - retrieve organizations with specific 'details' value") {
    ...
    expectMsgPF() {
      case FormatResult(entries) =>
        ...
        assert(entries.head.text.text.contains(""""name": "Denise Finch","""))
        assert(entries.head.text.text.contains(""""_id": 37"""))
    }
  }
}
